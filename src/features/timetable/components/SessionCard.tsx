...
const SessionCard: React.FC<SessionCardProps> = ({ session, isGrayedOut }) => {
  ...
  const speakerNames = session.talks && session.talks.length > 0
  ...
  return (
    <Link href={`/sessions/${session.id}`}>
      <div className={`bg-white rounded-lg p-4 mb-4 border-2 border-gray-800 font-google-sans cursor-pointer hover:shadow-lg transition-shadow ${isGrayedOut ? 'opacity-30' : ''}`}>
